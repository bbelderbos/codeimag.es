...
def upgrade():
    ...
    op.add_column('user', sa.Column('activation_key', sqlmodel.sql.sqltypes.AutoString(), nullable=False))
    ...
    op.create_index(op.f('ix_user_activation_key'), 'user', ['activation_key'], unique=False)
    op.create_index(op.f('ix_user_key_expires'), 'user', ['key_expires'], unique=False)
    op.create_index(op.f('ix_user_premium_day_limit'), 'user', ['premium_day_limit'], unique=False)
